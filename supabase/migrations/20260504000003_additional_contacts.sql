-- =============================================================================
-- Multiple contact persons per lead / client.
--
-- The primary contact stays in the existing first_name / last_name / email /
-- phone columns; we add a contact_info free-text column so accounting / sales
-- can capture role or scheduling notes for that contact.
--
-- Additional contacts live in a JSONB array (one row per extra person):
--   { "full_name": "...", "email": "...", "phone": "...", "info": "..." }
-- Validation is light by design — these are notes, not authoritative records.
-- =============================================================================
...
-- Patch convert_lead_to_client so the new fields carry over at conversion.
...
begin
  ...
  insert into public.deals (
    ...
    locked_at, locked_by, code, won_by_user_id, payment_method
  ) values (
    new_client_id,
