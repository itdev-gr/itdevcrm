...
-- hard-deletes the jobs. assigned_tasks cascade; deal_payment_lines.job_id is
-- nulled by its existing ON DELETE SET NULL FK (the billing lines + their amounts
-- are kept, just unlinked). Polymorphic job comments and attachments have no FK to
-- jobs, so they are removed explicitly here. The jobs after-delete trigger already
-- records each delete in activity_log.
...
-- Count of billing (payment) lines referencing a job, for the delete-confirmation
-- warning. Admin-only; returns 0 for non-admins. deal_payment_lines is the only
-- job-linked billing table in the current schema.
...
as $$
  select case when not public.current_user_is_admin() then 0 else
    (select count(*) from public.deal_payment_lines where job_id = p_job_id)::int
  end;
$$;
