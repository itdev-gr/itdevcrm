-- =============================================================================
-- Fix convert_lead_to_client: use array_append() instead of `array || text`.
-- The latter is ambiguous when Postgres resolves operator overloads — with
-- `text[] || 'literal'` the planner can prefer `text[] || text[]` (concat),
-- which forces `'literal'` to be cast to text[] via array-literal parsing,
-- producing the runtime error: "malformed array literal: contract_attachment_required".
-- array_append(text[], text) is unambiguous and correct.
-- =============================================================================
...
begin
  ...
  if contract_count = 0 then
    errors := array_append(errors, 'contract_attachment_required');
  end if;
  ...
  -- 4. Reassign collaboration entities
