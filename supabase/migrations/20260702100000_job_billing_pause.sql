...
-- =========================================================================
-- ---- Section 5: REVERT (verbatim — run via apply_migration to undo) -----
-- =========================================================================
-- CAVEAT: restoring the original CHECK is only valid while ZERO rows have
-- status='cancelled'. Before reverting, re-status or delete cancelled rows:
--   select count(*) from public.deal_payments where status='cancelled';
--
-- 1) Drop the RPCs:
--   drop function if exists public.job_pause_billing(uuid);
--   drop function if exists public.job_resume_billing(uuid);
--
-- 2) Restore the v1 index and drop v2:
--   create unique index if not exists deal_payments_recurring_period_key_unique
--     on public.deal_payments (deal_id, service_type, billing_type, start_date, end_date)
--     where billing_type in ('recurring_monthly','recurring_yearly')
--       and start_date is not null and end_date is not null;
--   drop index if exists public.deal_payments_recurring_period_key_unique_v2;
--
-- 3) Restore the original CHECK (requires zero cancelled rows, see caveat):
...
--
-- 4) Restore the 6 pre-patch function bodies (deployed prod snapshot,
--    captured 2026-07-02 before Section 3 was applied):
--
--   -- Pre-pause snapshot of deployed prod bodies (2026-07-02), for the S5 revert block
--   -- of 20260702100000_job_billing_pause.sql. Captured via pg_get_functiondef.
--   
--   CREATE OR REPLACE FUNCTION public.deal_next_due(p_deal_id uuid)
--    RETURNS date
--    LANGUAGE sql
...
--   ;
--   
--   -- CHECK constraint (pre-migration):
--   --   CHECK ((status = ANY (ARRAY['pending'::text, 'paid'::text, 'overdue'::text])))
--   -- Index (pre-migration):
--   --   CREATE UNIQUE INDEX deal_payments_recurring_period_key_unique ON public.deal_payments
--   --     USING btree (deal_id, service_type, billing_type, start_date, end_date)
--   --     WHERE ((billing_type = ANY (ARRAY['recurring_monthly'::text, 'recurring_yearly'::text]))
--   --       AND (start_date IS NOT NULL) AND (end_date IS NOT NULL))
