-- Lead → Won (conversion) now requires payment_method to be set on the lead.
-- The deal already enforced this for accounting stage moves; this closes the
-- corresponding gap on the sales side so accounting never inherits a deal
-- without a chosen method.
