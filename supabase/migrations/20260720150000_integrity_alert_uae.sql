-- 2026-07-20: United Arab Emirates joins Cyprus as a 0%-VAT country (owner decision).
-- The nightly accounting integrity check #3 (vat_missing) flags any job at 0% VAT whose
-- client is not Cyprus and whose deal is not cash-no-VAT. Without widening that condition,
-- every UAE job at its (correct) 0% VAT would raise a false "VAT missing" alert.
-- This recreates public.accounting_integrity_alerts() from the LIVE prod body (which had
-- drifted from the original migration file: check #20 off_board_job now also lists
-- 'maintenance'), changing ONLY check #3 so both 'cyprus' and 'united arab emirates'
-- (case-insensitive) are accepted at 0% VAT, plus the alert detail text.
-- create-or-replace preserves existing ACLs/ownership, so no re-grants here.
--
-- ROLLBACK: recreate the function with check #3's country guard restored to a single line:
--     and coalesce(c.country,'') not ilike 'cyprus'
--   and the detail text back to
--     'Job at 0% VAT but client is not Cyprus and deal is not cash-no-VAT'
--   (leave every other branch, including check #20's 'maintenance', unchanged).
...
begin
  ...
  with alerts (check_key, severity, category, subject_type,
    ...
    union all
    -- 3 vat_missing
    -- 0%-VAT countries (Cyprus + United Arab Emirates) legitimately bill at 0%, as do
    -- cash-no-VAT deals; only flag jobs at 0% VAT that fit neither exemption.
