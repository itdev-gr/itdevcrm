-- =============================================================================
-- 20260908170000_global_search_owner_only_jobs.sql
-- Συνοδεύει το 20260908160000: το global_search είναι SECURITY DEFINER και
-- παρακάμπτει το jobs RLS, οπότε το jobs branch χρειάζεται το ίδιο owner-only
-- φίλτρο για τα boards του owner_only_boards() (social_media, ads). Το
-- v_jobs_all (admin ∪ accounting_recurring/onboarding view) είναι ήδη ακριβώς
-- το escape set του conjunct (3) του νέου jobs_select.
--
-- Redefines global_search (last: 20260831120000_global_search_no_leads_for_accounting.sql
-- — verified no later redefinitions). Byte-identical except: the new
-- v_owner_only declare and one extra AND in the jobs branch. Pre/post
...
declare
  ...
  -- Owner-only boards (2026-09-08): non-admin members see only their own jobs.
  v_owner_only   text[]  := public.owner_only_boards();
...
begin
  ...
  with hits as (
    ...
    where j.archived = false
      ...
      and (v_jobs_all
           or j.service_type <> all (v_owner_only)
           or j.owner_user_id = v_uid)
      and (
...
-- ROLLBACK: re-apply the 20260831120000_global_search_no_leads_for_accounting.sql
-- definition (identical minus v_owner_only and the jobs-branch owner AND).
