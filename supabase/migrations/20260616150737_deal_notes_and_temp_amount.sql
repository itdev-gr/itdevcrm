-- Deal "Notes area" + temporary deal amount.
-- * temp_deal_amount: raw ClickUp "Deal Amount" (free text), shown next to the
--   payment method so accounting reconciles it manually (no per-service split).
-- * sales_note: the lead's "Sales Note" (leads.additional_notes) carried onto the
--   deal, shown in the deal Overview "Notes area" alongside the per-job notes.
alter table public.deals add column if not exists temp_deal_amount text;
alter table public.deals add column if not exists sales_note text;

-- Extend convert_lead_to_client so the lead's Sales Note (additional_notes)
-- carries onto the new deal. (Unchanged otherwise — same body as
-- 20260504000003_additional_contacts.sql.)
...
begin
  ...
  if l.payment_method is null or l.payment_method = '' then
    errors := array_append(errors, 'payment_method_required');
  end if;
...
-- ROLLBACK:
-- alter table public.deals drop column if exists temp_deal_amount, drop column if exists sales_note;
-- (and restore convert_lead_to_client to the body in 20260504000003_additional_contacts.sql — remove sales_note from the deals insert.)
