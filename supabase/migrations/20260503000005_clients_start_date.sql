-- =============================================================================
-- clients.start_date — the day the lead converted to Won and entered Accounting.
-- Auto-stamped by convert_lead_to_client at conversion time (= current_date).
-- Backfilled from the linked deal's actual_close_date for existing rows.
-- =============================================================================
alter table public.clients add column if not exists start_date date;

update public.clients c
  set start_date = d.actual_close_date
  from public.deals d
  where d.client_id = c.id and c.start_date is null and d.actual_close_date is not null;

-- Anything still null (created manually before the leads pipeline existed)
-- gets the row's created_at::date as a sensible fallback.
update public.clients
  set start_date = created_at::date
  where start_date is null;

-- Patch convert_lead_to_client to stamp start_date on the new client row.
...
begin
  ...
  insert into public.clients (
    ...
    l.industry, l.country, l.vat_number, l.website, l.owner_user_id, l.code, current_date
