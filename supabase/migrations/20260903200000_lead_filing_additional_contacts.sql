-- =============================================================================
-- 20260903200000_lead_filing_additional_contacts.sql
--
-- Owner decision 2026-09-03 (sales@ capture review): inbound from a lead's
-- SECONDARY address was dropped because the lead branch matched only
-- leads.email. Adds the same additional_contacts[].email fallback the client
-- branch has (20260827120000). Base body: 20260827120000 emission — verify
-- md5(pg_get_functiondef) against the deploy output before/after (drift log).
-- Everything outside the lead branch is byte-identical.
--
-- ROLLBACK: re-run the CREATE OR REPLACE from 20260827120000.
-- =============================================================================
...
begin
  ...
  if v_client is null then
    ...
    if v_client is not null then
      ...
      -- 2026-09-03: leads writing from a secondary address — same fallback the
      -- client branch got on 2026-08-27, now over leads.additional_contacts.
      if v_lead is null then
        select l.id into v_lead from leads l
          cross join lateral jsonb_array_elements(coalesce(l.additional_contacts, '[]'::jsonb)) ac
         where lower(coalesce(ac->>'email','')) = v_client_email
           and l.converted_at is null and l.archived = false
         order by l.created_at desc limit 1;
      end if;
