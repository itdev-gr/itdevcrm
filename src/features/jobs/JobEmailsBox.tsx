...
import { useJobEmails } from './hooks/useJobEmails';
...
export function JobEmailsBox({ jobId, clientId }: { jobId: string; clientId: string | null }) {
  const { data: rows = [], isLoading } = useJobEmails(jobId, clientId);
  ...
  return (
    <div className="rounded-xl border border-border/60 bg-card p-3 shadow-sm">
      ...
      {isLoading ? (
        ...
        <p className="text-xs text-muted-foreground">No emails sent for this job yet.</p>
      ) : (
