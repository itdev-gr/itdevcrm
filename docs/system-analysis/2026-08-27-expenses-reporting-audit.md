...
**Headline exposure, in four numbers (2026-08-27):** the YTD expense figure shown on every visit to `/accounting/report` reads **€0.00 instead of €25,721.40** because 100% of this year's paid expenses silently fall past a 1,000-row fetch cap (E22) — the summary object's net-basis profit is consequently overstated by **€5,504.16** (the strip prints the gross equivalents; the gross-basis error differs and needs a live query to pin — plausibly ~€1.5k, since 737 of 895 paid rows carry 24% VAT). The P&L itself recognises only **€25,721.40 of €82,739.06** of real expenses — 31.1% — leaving **€57,017.66 pending**, of which **€44,463.76 is already past its own due date** (E10). A further **€8,250.80** across 13 payment rows has no date of any kind and cannot be attributed to any month (E18). And the accounting PDF the owner wants to rely on prints at most **40 rows per side**, in a font that cannot render Greek at all (E28, E29).
...
This audit is the expense-side companion to `docs/system-analysis/2026-08-26-payment-system-audit.md` (the revenue side, findings A0–A8 / B1–B17). Where that audit already owns a finding — ledger mutability (B2 and its underlying F46), the cash/no-VAT charge (A0) — this report cites it and does not re-derive it. The frontend permission sweep (A8) is different: that audit marked it "not systematically re-audited," so this report's own D5 records a fresh, live-verified instance rather than assuming it identical.
...
- **The repo is trustworthy.** `ensure_recurring_expenses()` is byte-for-byte its newest migration in prod (E6). `accounting_ledger_v` and `accounting_pl_summary_v` show no drift, verified against `pg_get_viewdef` (structurally identical; md5s differ from migration text only by Postgres's re-emission) (E7, E8). Zero live-vs-repo drift of substance. Where a document and the database disagree, the database is right.
...
#### E22. The always-on YTD strip drops every expense in the year — YTD expense reads €0.00 instead of €25,721.40, every visit
...
**Evidence (2026-08-27).** The 2026 YTD range is already **1,284 ledger rows** — over the cap today. With no `ORDER BY` anywhere (the view body has none either), the planner runs the `deal_payments` arm of the `UNION ALL` first and in full — confirmed by live `EXPLAIN` — so the income arm alone exhausts the 1,000-row cap before the expense arm is reached (the year's income arm is **1,152 rows**). Reproducing the exact unordered `LIMIT 1000` split with `row_number() over ()`: **every single `direction='out'` row for the year is dropped — all 32 paid (€25,721.40) and all 100 pending — plus 90 paid income rows (€20,217.24)**.

| | summary object, net basis | true (E16 baseline) |
|---|---|---|
| YTD income net | ≈ €203,373.92 | €223,591.16 |
| YTD expense net | **€0.00** | €25,721.40 |
| YTD net profit | ≈ €203,373.92 | €197,869.76 |

**Net effect:** the certain, un-nettable fact is that the YTD expense figure reads **€0.00 instead of €25,721.40** on every visit. The summary object's net-basis profit is consequently overstated by **€5,504.16** — but the strip the owner actually sees prints the *gross* equivalents (`totalIncomeGross`/`totalExpenseGross`/`netProfitGross`, `ReportHeader.tsx:175/180/185`), so the on-screen gross-basis error is a different, smaller number that needs a live query to pin (plausibly ~€1.5k, since 737 of the 895 paid income rows carry 24% VAT). Selecting the `this_year` preset explicitly is *also* over the cap but fails differently — `useLedger` does call `.order('event_date', desc)`, so it deterministically drops the **284 oldest events** (2026-01-23 → 2026-06-22). The two would visibly disagree with each other for the same selected range.
...
**D4 — What should happen to the 13 dateless payment rows?** (E18, €8,250.80, 2026-08-27.) 13 `deal_payments` rows have **both** `paid_at` and `start_date` NULL, so `accounting_ledger_v.period` is NULL and they vanish from any month-grouped query. All 13 are `pending`, so they correctly contribute €0 to the paid-only P&L — this is a completeness gap, not a math error — but any "expected income" or aging view built on the ledger silently loses €8,250.80 of real receivables, up to €2,000 on a single row. Their `created_at` values run 2026-06-22 → 2026-08-06: **10 of the 13 created on 2026-06-22 across about an hour and three quarters**, one more on 2026-06-25, and the remaining two spread across dates running through 2026-08-06 — a batch creation event followed by a trickle, not independent one-offs. `start_date` has no `NOT NULL` constraint (`20260503000010_deal_payments.sql:16`), so nothing prevents more. Backfill the dates, then decide whether to add the constraint plus a required UI field. **A monthly-subtotal PDF structurally cannot show these rows until they have a date.**
...
2. **D5 / E26 — the permission decision, enforced.** The new endpoint returns the entire income ledger in one response. `contract-pdf.ts` relies on RLS for its authorisation, and `deal_payments` RLS **does not restrict to admins**. The endpoint must therefore carry an **explicit admin check of its own** (`current_user_is_admin()` via the user-scoped client) before it fetches anything — not inherited RLS. Settle D5 first so the check encodes the intended policy. If the check were skipped and the endpoint simply inherited RLS instead, a non-admin caller would not be denied — they would silently get a **half-empty document**: the income arm populated (via the broad `deal_payments` RLS that 20 non-admin accounts already satisfy, D5) and the expense arm empty (via the admin-only `expenses` RLS), with nothing on the page to say so. That failure mode — a document that looks complete but isn't — is exactly why the explicit admin check must come first, ahead of any fetch.
...
| check | result |
|---|---|
| `accounting_ledger_v` / `accounting_pl_summary_v` / `ensure_recurring_expenses()` vs. newest migration | **0 drift** — `ensure_recurring_expenses()` is byte-for-byte its latest migration (E6); the two views show no drift, verified against `pg_get_viewdef` (structurally identical; md5s differ from migration text only by Postgres's re-emission) (E7, E8) |
...
- **Corrections from review applied.** E18's clustering is stated as **10 of 13 rows on 2026-06-22 over ~1h45m, 1 more on 2026-06-25, and the remaining 2 through 2026-08-06** — all 13 accounted for — not the findings file's "12 of 13 in one hour". The prepay RPC citation is `20260716220000_accounting_prepay_months.sql:75`. The Dashboard `last_6_months` loss is stated as 2,365 of 3,365 (70%) from the row counts, rather than the findings file's rounded 72%.
