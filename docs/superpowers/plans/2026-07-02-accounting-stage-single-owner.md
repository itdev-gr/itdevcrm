...
- **The single rule (exact boundaries):** for a deal whose current accounting stage ∈ {`awaiting_payment`,`paid_in_full`} AND `payment_method IS NOT NULL`, with `v_next_due` = `min(start_date)` over its `deal_payments` where `status NOT IN ('paid','cancelled')` and `start_date IS NOT NULL`: `v_next_due IS NULL → paid_in_full`; `v_next_due < current_date → on_hold`; `v_next_due <= current_date + 7 → awaiting_payment`; else `paid_in_full`. On-Hold uses **strict `<`** (a charge due *today* → awaiting, not on_hold — this is why no grace is needed).
- **DECISION B — never auto-lift a hold:** a deal whose current stage is **`on_hold` is SKIPPED** by the rule — it keeps that deal's jobs blocked but never moves it out. Lifting a hold is the accountant's action (their manual move off On Hold unblocks the jobs via the unchanged `deals_hold_jobs_on_stage_change`). This protects deliberate holds (e.g. a blocked client with nothing owed) from being silently returned to Paid In Full. So the rule only *moves* deals that are in `awaiting_payment` or `paid_in_full`; it moves them **into** `on_hold` on overdue but never **out** of it. (Implemented in migration `20260702150150`.)
