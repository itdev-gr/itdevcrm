...
## SMOKE-TEST RE-RUN (verifying the fixes) — caught + fixed 2 more issues
Ran a live end-to-end smoke test (3 deals: closed / ended-job / active) exercising all 6 fixes. Results: fix 1 closed→no bill ✓, fix 2 closed→no rehold ✓, fix 3 end_job→no bill ✓, fix 4 emails ✓, fix 6 keys self-heal (`1 / web_seo` on a NULL insert) ✓. **Two new findings, both fixed:**
- **Regression in BUG 4's fix** (`20260619150000`): matching idempotency on `service_type` was wrong — the reseed inserts the ORIGINAL period with NULL `service_type` while the derive-trigger gives successors a real type, so they never matched → duplication resumed. Fixed by matching on `service_index` ONLY (NULL-safe) + a `pg_advisory_xact_lock` to serialize concurrent cron runs (board mounts + daily job). Verified: 6 cron runs → 0 dups.
- **Reseed inserts duplicate periods DIRECTLY** (bypassing the cron) (`20260619160000`): a BEFORE INSERT guard on `deal_payments` silently drops an incoming recurring row whose `(deal_id, service_index, billing_type, start_date, end_date)` already exists — returns NULL so it no-ops without erroring (won't break the active reseed). Verified: duplicate insert dropped; with reseed + cron both running, `dup_groups` holds at 0.
