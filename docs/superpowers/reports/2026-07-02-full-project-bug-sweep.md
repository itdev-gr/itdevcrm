...
1. ✅ DONE 2026-07-01 — **P1 — GRANT-boundary regression: 107 anon-executable SECURITY DEFINER functions.** Execute the 2026-06-28 remediation plan + default-privileges hardening. This is the only finding with real security weight; most functions gate internally on `auth.uid()`, but the surface keeps growing. Fixed by supabase/migrations/20260701230000_revoke_secdef_fn_grants.sql + 20260701231000_default_privs_global_public_revoke.sql (49 RPCs keep authenticated; 58 internal/trigger fns closed; anon-executable secdef count now 0; defaults hardened globally). Verified by catalog assertions + full admin/sales browser smoke.
